    ...
    <c:if test="${isEmpty == 'false'}">
        <table width="100%" border=1>
            ...
            <tr>
                ...
                <td><a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?teacherId=<%=teacher.getTeacherId()%>">修改</a></td>
            </tr>
            <%
                }
            %>
        ...
        <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=1">首页</a>
        <c:choose>
            <c:when test="${page.pageNow - 1 > 0}">
                <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=${page.pageNow - 1}">上一页</a>
            </c:when>
            <c:when test="${page.pageNow - 1 <= 0}">
                <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=1">上一页</a>
            </c:when>
        </c:choose>

        <c:choose>
            <c:when test="${page.totalPageCount==0}">
                <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=${page.pageNow}">下一页</a>
            </c:when>
            <c:when test="${page.pageNow + 1 < page.totalPageCount}">
                <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=${page.pageNow+1}">下一页</a>
            </c:when>
            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">
                <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=${page.totalPageCount}">下一页</a>
            </c:when>
        </c:choose>

        <c:choose>
            <c:when test="${page.totalPageCount==0}">
                <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=${page.pageNow}">尾页</a>
            </c:when>
            <c:otherwise>
                <a href="${pageContext.request.contextPath }/admin/updateSingleTeacher?pageNow=${page.totalPageCount}">尾页</a>
            </c:otherwise>
        </c:choose>
    </c:if>
</form>
